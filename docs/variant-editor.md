...
The Variant Editor lets you take a single Rig and turn it into a whole cast of characters, containers, or props - reskinning its Bones with different sprites and tuning its stats, without touching the underlying skeleton or Animations. See the [Glossary](glossary.md) for a refresher on Variants if you're new to the concept, and check out the "Darktone" and "Skele:Bruiser" examples on the [engine overview page](README.md) for a sense of what Variants make possible.
...
{% hint style="danger" %}
Screenshot needed: the Variant list.
{% endhint %}
...
Selecting a Bone from the bone list on this page lets you override how that Bone looks *for this Variant only*, without affecting other Variants that share the same Rig:
...
{% hint style="danger" %}
Screenshot needed: the bone list with sprite override fields.
{% endhint %}
...
{% hint style="danger" %}
Screenshot needed: the stat fields panel.
{% endhint %}
